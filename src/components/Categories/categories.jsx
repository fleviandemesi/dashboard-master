...
const Categories = () => {
  ...
  const handleSearch = (value) => {
    ...
    const filterResult = categorys && categorys.filter((item) => {
      ...
      const categoryId = item.category_id ? item.category_id.toString().toLowerCase() : '';
      const categoryName = item.category_name ? item.category_name.toString().toLowerCase() : '';
  
      return (
        categoryId.includes(lowerCaseQuery) ||
  ...
  return (
    <div>
      ...
      <div className="card">
        ...
        <input type="text" placeholder="Search by ID or Name" className="form1" value={query} onChange={(e) => handleSearch(e.target.value)} />
        {categorys?.length > 0 ? (
